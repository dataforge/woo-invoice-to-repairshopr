...
class WIR_Updater {
	...
	const GITHUB_REPO = 'dataforge/woo-invoice-to-repairshopr';
	const SLUG        = 'woo-invoice-to-repairshopr';
	const CACHE_KEY   = 'wir_github_release';
	...
	public static function check_update( $update, $plugin_data, $plugin_file, $locales ) {
		if ( plugin_basename( WIR_PLUGIN_FILE ) !== $plugin_file ) {
			return $update;
		}
	...
	public static function fix_directory( $result, $options ) {
		...
		if ( ! isset( $options['plugin'] ) || plugin_basename( WIR_PLUGIN_FILE ) !== $options['plugin'] ) {
			return $result;
		}
	...
	public static function handle_check_updates() {
		...
		check_admin_referer( 'wir_check_updates' );
		...
		wp_safe_redirect( add_query_arg(
			...
			admin_url( 'admin.php?page=woo-invoice-to-repairshopr' )
		) );
		exit;
	}
